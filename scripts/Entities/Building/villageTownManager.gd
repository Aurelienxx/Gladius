...
func _ready() -> void:
	...
	- Unit_Clicked : désélectionne les autres villages
...
func _on_shape_clicked():
	...
	# _show_villagetown_ui()  # Affichage de l'UI spécifique au village (ex : menu d'amélioration)


func _on_shape_attack_clicked():
	"""
	Émet un signal global lorsqu’un clic droit est fait sur le village,
	permettant à d’autres entités de réagir (ex : attaque, interaction spéciale).
	"""
	GlobalSignal.Unit_Attack_Clicked.emit(villageTown_body, null)
	# Ici, une logique d’attaque contre le village peut être ajoutée.
